...
gmsh.initialize()
gmsh.open("/home/salaml/ablateInputs/geom/CombustionChamberV5_area_test2.msh")
...
for l in range(len(elementTags)):
    for elementId in elementTags[l]:
        ...
        # Get the element
        elementType, nodeTags, elementDim, elementTag = gmsh.model.mesh.getElement(elementId)  # elementType, nodeTags, dim, tag
        if elementId > 212:
            if len(nodeTags) > 4:
                print("More than four nodes")
                print("The nodeTags for element ", elementId, " are: ", nodeTags)
# faceTags, faceNodes = gmsh.model.mesh.getAllFaces(4)
# print("faceTags: ", faceTags)
...
#         elementType, nodeTags, elementDim, elementTag = gmsh.model.mesh.getElement(elementId)  # elementType, nodeTags, dim, tag
        # if elementType != 4:
        #     print("Error ", elementId, " not tetrahedral, but has element type: ", elementType)

        # if elementTag > 212:
            # gmsh.model.mesh.getFaces(4,)
            # print(
